#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Clasificador de Temas para Comentarios de Campañas
Personalizable por campaña/producto
"""
import re
from typing import Callable


def create_topic_classifier() -> Callable[[str], str]:
    """
    Clasificador especializado para comentarios de la campaña de Avena Alpina.
    """

    def classify_topic(comment: str) -> str:
        comment_lower = str(comment).lower()

        # ---------------------------------------------------------
        # CATEGORÍA 1: Competencia / Comparación con otras marcas
        # ---------------------------------------------------------
        if re.search(
            r'avena torres|torres g[oó]mez|colanta|alpina vs|mejor que la avena|'
            r'otra marca|competencia|marca\s+\w+',
            comment_lower
        ):
            return 'Competencia / Comparación con otras marcas'

        # ---------------------------------------------------------
        # CATEGORÍA 2: Preguntas sobre el Producto
        # ---------------------------------------------------------
        if re.search(
            r'\bprecio\b|\bcu[aá]nto vale\b|d[oó]nde\b|comprar|'
            r'c[oó]mo consigo|disponible|tiendas|sirve para|'
            r'c[oó]mo se toma|tiene az[uú]car|valor',
            comment_lower
        ):
            return 'Preguntas sobre el Producto'

        # ---------------------------------------------------------
        # CATEGORÍA 3: Ingredientes y Salud
        # (incluye especificaciones como "conservantes")
        # ---------------------------------------------------------
        if re.search(
            r'conservantes|preservantes|aditivos|saludable|'
            r'az[uú]car añadid|demasiado dulce|muy dulce|bajen.*az[uú]car|'
            r'ingredientes|lactosa',
            comment_lower
        ):
            return 'Ingredientes y Salud'

        # ---------------------------------------------------------
        # CATEGORÍA 4: Opinión General del Producto
        # (solo si NO es competencia — ya filtrado arriba)
        # ---------------------------------------------------------
        if re.search(
            r'rico|delicioso|me encanta|bueno|espectacular|'
            r'me gusta|qué rica|que rica|riquísima',
            comment_lower
        ):
            return 'Opinión General del Producto'

        # ---------------------------------------------------------
        # CATEGORÍA 5: Fuera de Tema / No Relevante
        # ---------------------------------------------------------
        if (
            re.search(r'am[eé]n|gracias|jajaja|jes[uú]s|bendiciones', comment_lower)
            or len(comment_lower.split()) < 3
        ):
            return 'Fuera de Tema / No Relevante'

        # ---------------------------------------------------------
        # CATEGORÍA DEFAULT
        # ---------------------------------------------------------
        return 'Otros'

    return classify_topic


# ============================================================================
# METADATA DE LA CAMPAÑA (OPCIONAL)
# ============================================================================

CAMPAIGN_METADATA = {
    'campaign_name': 'Alpina - Kéfir',
    'product': 'Kéfir Alpina',
    'categories': [
        'Preguntas sobre el Producto',
        'Comparación con Kéfir Casero/Artesanal',
        'Ingredientes y Salud',
        'Competencia y Disponibilidad',
        'Opinión General del Producto',
        'Fuera de Tema / No Relevante',
        'Otros'
    ],
    'version': '1.0',
    'last_updated': '2025-11-20'
}


def get_campaign_metadata() -> dict:
    """Retorna metadata de la campaña"""
    return CAMPAIGN_METADATA.copy()
